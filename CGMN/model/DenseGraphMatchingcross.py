...
from model.DenseGGNNcross import DenseGGNNcross
...
class HierarchicalGraphMatchNetworkcross(torch.nn.Module):
    def __init__(self, node_init_dims, arguments, device):
        super(HierarchicalGraphMatchNetworkcross, self).__init__()
        ...
        conv_layer_constructor = {
            ...
            "ggnn": dict(constructor=DenseGGNNcross, kwargs=ggnn_parameters),
        }
        ...
        if self.args.task.lower() == "regression":
            ...
            self.predict_fc1 = nn.Linear(
                int(self.hidden_size * 2 * factor), int(self.hidden_size * factor)
            )
            self.predict_fc2 = nn.Linear(
                int(self.hidden_size * factor), int((self.hidden_size * factor) / 2)
            )
            self.predict_fc3 = nn.Linear(
                int((self.hidden_size * factor) / 2),
                int((self.hidden_size * factor) / 4),
            )
    ...
    def forward(self, batch_x_p, batch_adj_p):  # , batch_x_h, , batch_adj_h

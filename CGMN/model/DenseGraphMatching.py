...
class HierarchicalGraphMatchNetwork(torch.nn.Module):
    def __init__(self, node_init_dims, arguments, device):
        ...
        self.gcn_last_filter = self.gcn_filters[
            -1
        ]  # last filter size of node embedding layer

        gcn_parameters = [
            dict(
                in_channels=self.gcn_filters[i - 1],
                out_channels=self.gcn_filters[i],
                bias=True,
            )
            for i in range(1, self.gcn_numbers)
        ]
        gcn_parameters.insert(
            0,
            dict(
                in_channels=node_init_dims, out_channels=self.gcn_filters[0], bias=True
            ),
        )
        ...
        gin_parameters = [
            dict(
                nn=nn.Linear(
                    in_features=self.gcn_filters[i - 1],
                    out_features=self.gcn_filters[i],
                )
            )
            for i in range(1, self.gcn_numbers)
        ]
        gin_parameters.insert(
            0,
            {
                "nn": nn.Linear(
                    in_features=node_init_dims, out_features=self.gcn_filters[0]
                )
            },
        )

        ggnn_parameters = [
            dict(out_channels=self.gcn_filters[i]) for i in range(self.gcn_numbers)
        ]
        ...
        if self.global_flag is True:
            ...
            if self.global_agg.lower() == "max_pool":
                ...
                self.global_fc_agg = nn.Linear(
                    self.gcn_last_filter, self.gcn_last_filter
                )
                ...
                self.global_lstm_agg = nn.LSTM(
                    input_size=self.gcn_last_filter,
        ...
        self.perspectives = (
            self.args.perspectives
        )  # number of perspectives for multi-perspective matching function
        if self.args.match.lower() == "node-graph":
            self.mp_w = nn.Parameter(
                torch.rand(self.perspectives, self.gcn_last_filter)
            ...
            self.bilinear = nn.Bilinear(
                in1_features=self.gcn_last_filter,
                in2_features=self.gcn_last_filter,
                out_features=self.perspectives,
            )
            self.lstm_input_size = self.perspectives
        elif self.args.match.lower() == "concat":
            self.nn = nn.Linear(
                in_features=2 * self.gcn_last_filter, out_features=self.perspectives
            )
            self.lstm_input_size = self.perspectives
            ...
            self.lstm_input_size = self.perspectives
            self.nn = nn.Linear(
                in_features=self.gcn_last_filter * 2, out_features=self.perspectives
            )

        # ---------- Aggregation Layer ----------
        self.hidden_size = (
            self.args.hidden_size
        )  # fixed the dimension size of aggregation hidden size
        # match aggregation
        if self.args.match_agg.lower() == "bilstm":
            ...
            self.args.match_agg.lower() == "fc_avg"
            or self.args.match_agg.lower() == "fc_max"
        ):
            self.fc_agg = nn.Linear(self.lstm_input_size, self.lstm_input_size)
        elif (
            self.args.match_agg.lower() == "avg" or self.args.match_agg.lower() == "max"
        ):
            pass
        else:
            raise NotImplementedError
        ...
        if self.args.task.lower() == "regression":
            ...
            self.predict_fc1 = nn.Linear(
                int(self.hidden_size * 2), int(self.hidden_size)
            )
            self.predict_fc2 = nn.Linear(
                int(self.hidden_size), int((self.hidden_size) / 2)
            )
            self.predict_fc3 = nn.Linear(
                int((self.hidden_size) / 2), int((self.hidden_size) / 4)
            )
    ...
    def global_aggregation_info(self, v, agg_func_name):
        ...
        if agg_func_name.lower() == "max_pool":
            ...
            agg_v = (
                agg_v_last.permute(1, 0, 2)
                .contiguous()
                .view(-1, self.gcn_last_filter * 2)
            )
        else:
            raise NotImplementedError
        return agg_v
    ...
    def drop_feature(self, x, drop_prob):
        for i in range(x.size()[0]):
            ...
            drop_mask = (
                torch.empty(
                    (x[i].size(1),), dtype=torch.float32, device=x[i].device
                ).uniform_(0, 1)
                < drop_prob
            )
            x[i] = x[i].clone()
    ...
    def multi_perspective_match_func(self, v1, v2, w):
        ...
        """
        w = (
            w.transpose(1, 0).unsqueeze(0).unsqueeze(0)
        )  # (1,      1,  dim, perspectives)
        v1 = w * torch.stack(
            [v1] * self.perspectives, dim=3
        )  # (batch, len, dim, perspectives)
        v2 = w * torch.stack(
            [v2] * self.perspectives, dim=3
        )  # (batch, len, dim, perspectives)
    ...
    def forward_dense_gcn_layers(self, feat, adj):
        ...
        for i in range(1, self.gcn_numbers + 1):
            feat_out = functional.relu(
                getattr(self, "gc{}".format(i))(
                    x=feat_in, adj=adj, mask=None, add_loop=False
                ),
                inplace=True,
            )
            feat_out = functional.dropout(
                feat_out, p=self.dropout, training=self.training
            )
            feat_in = feat_out
        return feat_out
    ...
    def forward_cross_gcn(self, feat, adj):
        ...
        for i in range(1, self.gcn_numbers + 1):
            feat_out = functional.relu(
                getattr(self, "gc_cross{}".format(i))(
                    x=feat_in, adj=adj, mask=None, add_loop=False
                ),
                inplace=True,
            )
    ...
    def forward(self, batch_x_p, batch_adj_p):
        ...
        feature_p1 = self.forward_dense_gcn_layers(
            feat=batch_x_p, adj=batch_adj_p
        )  # (batch, len_p, dim)
    ...
    def aug_random_edge(self, input_adj, drop_percent):
        ...
        b = np.where(
            ...
            np.random.choice(
                2,
                (input_adj.shape[0], input_adj.shape[0]),
                p=[drop_percent, 1 - drop_percent],
            ),
            input_adj,
        )
        drop_num = len(input_adj.nonzero()[0]) - len(b.nonzero()[0])
        mask_p = drop_num / (
            input_adj.shape[0] * input_adj.shape[0] - len(b.nonzero()[0])
        )
        c = np.where(
            b == 0,
            np.random.choice(
                2, (input_adj.shape[0], input_adj.shape[0]), p=[1 - mask_p, mask_p]
            ),
            b,
        )
    ...
    def matching_layer_cross(self, feature_p, feature_h):
        ...
        att_mean_h = self.div_with_small_value(
            attention_h.sum(dim=2), attention.sum(dim=2, keepdim=True)
        )  # (batch, len_p, dim)
        att_mean_p = self.div_with_small_value(
            attention_p.sum(dim=1), attention.sum(dim=1, keepdim=True).permute(0, 2, 1)
        )  # (batch, len_h, dim)
    ...
    def matching_layer(self, feature_p, feature_h):
        ...
        # ---------- Matching Layer ----------
        if self.args.match.lower() == "node-graph":
            multi_p = self.multi_perspective_match_func(
                v1=feature_p, v2=att_mean_h, w=self.mp_w
            )
            multi_h = self.multi_perspective_match_func(
                v1=feature_h, v2=att_mean_p, w=self.mp_w
            )
        elif self.args.match.lower() == "bilinear":
            ...
            l2_dist_p = functional.pairwise_distance(
                feature_p_reshape, att_mean_h_reshape
            ).reshape(batch_size, -1, 1)
            cosine_dist_p = functional.cosine_similarity(
                feature_p_reshape, att_mean_h_reshape
            ).reshape(batch_size, -1, 1)
            l2_dist_h = functional.pairwise_distance(
                feature_h_reshape, att_mean_p_reshape
            ).reshape(batch_size, -1, 1)
            cosine_dist_h = functional.cosine_similarity(
                feature_h_reshape, att_mean_p_reshape
            ).reshape(batch_size, -1, 1)
            ...
            multi_p = functional.relu(
                self.nn(
                    torch.cat((feature_p - att_mean_h, feature_p * att_mean_h), dim=-1)
                )
            )
            multi_h = functional.relu(
                self.nn(
                    torch.cat((feature_h - att_mean_p, feature_h * att_mean_p), dim=-1)
                )
            )
        else:
            raise NotImplementedError
    ...
    def ginf(self, feature_p, feature_p0, global_agg):
        global_gcn_agg_p = self.global_aggregation_info(
            v=feature_p0, agg_func_name=global_agg
        )
    ...
    def GED_semi_loss(
        ...
        loss1 = -torch.log((x1 + x2) / (x1 + x2 + x3 + x4 + x5 + x6))

        return (
            loss2 * self.mse_loss
            + loss1 * self.node_graph_loss
            + loss0 * self.graph_graph_loss,
            simpos,
            simneg,
        )
    ...
    def GED_loss(
        ...
        for i in range(lengh):
            ...
            if batch_size == 0:
                l1, simpos1, simneg1 = self.GED_semi_loss(
                    h1, h2, hp1, hp2, nh1, nh2, nhp1, nhp2, y_pos[i], y_neg[i]
                )
                l2, simpos2, simneg2 = self.GED_semi_loss(
                    h2, h1, hp2, hp1, nh2, nh1, nhp2, nhp1, y_pos[i], y_neg[i]
                )
    ...
    def prediction(self, feature_p, feature_h, y):
        ...
        if self.args.match_agg.lower() == "bilstm":
            _, (agg_p_last, _) = self.agg_bilstm(
                feature_p
            )  # (batch, seq_len, l) -> (2, batch, hidden_size)
            agg_p = (
        ...
        if self.global_flag is True:
            global_gcn_agg_p = self.global_aggregation_info(
                v=feature_p, agg_func_name=self.global_agg
            )
            global_gcn_agg_h = self.global_aggregation_info(
                v=feature_h, agg_func_name=self.global_agg
            )
...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model):
        ...
        if self.verbose:
            print(
                f"Validation score decreased ({self.val_loss_min:.6f} --> {val_loss:.6f}).  Saving model ..."
            )

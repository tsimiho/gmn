...
node_feat_name_dict = {"aids700nef": "type", "linux": None, "imdbmulti": None}
...
class GEDDataset(object):
    def __init__(self, ged_main_dir, args):
        ...
        data = simgnn_utils.SiameseDataSet(
            data_save_folder=os.path.join(self.ged_main_dir, "data"),
            data_set_name=args.dataset,
            validation_ratio=0.25,
            node_feat_name=node_feat_name_dict[args.dataset],
            node_encoder_name=node_encoder_name_dict[args.dataset],
        )
        distance = simgnn_utils.DistanceCalculator(
            root_folder=os.path.join(self.ged_main_dir, "save"),
            data_name=data.data_set_name,
        )
        ...
        max_nr_nodes = max(
            self.train_val_max_number_of_nodes, self.testing_max_number_of_nodes
        )
        self.train_val_graphs = self.preprocess(data.train_val_gs, max_nr_nodes)
        self.training_graphs = self.preprocess(
            data.train_gs, self.data.train_val_max_number_of_nodes
        )
        self.val_graphs = self.preprocess(
            data.val_gs, self.data.train_val_max_number_of_nodes
        )
        self.testing_graphs = self.preprocess(data.test_gs, max_nr_nodes)

        true_result_test_train = simgnn_utils.load_result(
            dataset=data.data_set_name.lower(),
            model="astar",
            result_folder=os.path.join(self.ged_main_dir, "result"),
            sim=None,
            sim_mat=None,
            dist_mat=None,
            row_graphs=self.testing_graphs,
            col_graphs=self.train_val_graphs,
            scale=1.0,
        )
        self.ground_truth = true_result_test_train.dist_norm_mat_
        self.training_triples = self._load_train_triples(
            dist_calculator=self.dist_calculator
        )
        self.validation_triples = self._load_val_train_triples(
            dist_calculator=self.dist_calculator
        )
    ...
    def extract_test_matrices(self, row):
        ...
        return (
            np.array(batch_rows_feature),
            np.array(batch_rows_adjacent),
            np.array(batch_rows_mask),
            np.array(batch_cols_feature),
            np.array(batch_cols_adjacent),
            np.array(batch_cols_mask),
        )
    ...
    def get_training_batch(self):
        ...
        return (
            np.array(all_feat_1),
            np.array(all_adj_1),
            np.array(all_masks_1),
            np.array(all_feat_2),
            np.array(all_adj_2),
            np.array(all_masks_2),
            np.array(ged_pairs),
        )
    ...
    def _graph_feature_adj(self, gs, max_nodes):
        ...
        adj_padded[: adj_normalized.shape[0], : adj_normalized.shape[1]] = (
            adj_normalized
        )
    ...
    def _load_val_train_triples(self, dist_calculator):
        ...
        for m in range(val_length):
            ...
            for n in range(train_length):
                g2 = self.training_graphs[n]
                dist, norm_dist = dist_calculator.calculate_distance_btw_pairs(
                    g1, g2, 1.0
                )
...
class CFGDataset(object):
    def __init__(self, data_dir, batch_size):
        ...
        (
            ...
            graphs_test,
            classes_test,
        ) = self.partition_graph_dataset(self.gs, self.classes, [0.1, 0.1, 0.8], perm)

        self.graph_train = graphs_train
        self.classes_train = classes_train
        self.graph_test = graphs_test
        self.classes_test = classes_test

        print(
            "{} Train: {} graphs, {} functions".format(
                data_dir, len(graphs_train), len(classes_train)
            )
        )
        print(
            "{} Dev  : {} graphs, {} functions".format(
                data_dir, len(graphs_dev), len(classes_dev)
            )
        )
        print(
            "{} Test : {} graphs, {} functions".format(
                data_dir, len(graphs_test), len(classes_test)
            )
        )

        if os.path.isfile(os.path.join(self.data_dir, "valid.json")):
            os.remove(os.path.join(self.data_dir, "valid.json"))

        self.valid_epoch, valid_ids = utils.generate_epoch_pair(
            graphs_dev, classes_dev, batch_size, output_id=True
        )
        ...
        if os.path.isfile(os.path.join(self.data_dir, "test.json")):
            os.remove(os.path.join(self.data_dir, "test.json"))

        self.test_epoch, test_ids = utils.generate_epoch_pair(
            graphs_test, classes_test, batch_size, output_id=True
        )
        ...
        for file in os.listdir(self.data_dir):
            ...
            with open(f_name) as inf:
                for line in inf:
                    ...
                    cur_graph = utils.graph(
                        node_num=g_info["n_num"], label=label, name=g_info["src"]
                    )

...
class CFGTrainer(object):
    def __init__(
        ...
        self.model = HierarchicalGraphMatchNetwork(
            node_init_dims=node_init_dims, arguments=args, device=device
        ).to(device)
        # print(self.model)
        self.modelcross = HierarchicalGraphMatchNetworkcross(
            node_init_dims=node_init_dims, arguments=args, device=device
        ).to(device)
    ...
    def fit(self):
        ...
        for i in range(1, self.max_epoch + 1):  # for i in range(1, self.max_epoch + 1):
            ...
            # validation
            valid_auc, valid_lr = self.testing(
                self,
                model=self.model,
                eval_epoch_data=self.epoch_data_valid,
                device=self.device,
            ...
            write_log_file(
                self.log_file,
                "EPOCH {0}/{1}:\t Contrastive loss = {2} @ {3}".format(
                    i, self.max_epoch, loss_avg, datetime.now()
                ),
            )
            ...
            if best_val_auc is None or best_val_auc < valid_auc:
                write_log_file(
                    self.log_file,
                    "\tvalidation AUC increased ({} ---> {}), and saving the model ... ".format(
            ...
            if best_LR_auc is None or best_LR_auc < valid_lr:
                write_log_file(
                    self.log_file,
                    "\tvalidation AUC with LR increased ({} ---> {}), and saving the model ... ".format(
    ...
    def emb(self, model, graphs, classes, batch_size, device, load_data=None):
        ...
        for index in perm:
            ...
            feature_p = model(
                batch_x_p=feature_p_init.clone(), batch_adj_p=adj_p.clone()
            )
            feature_p1 = copy.copy(feature_p)
            feature_p_list.append(feature_p1)

            feature_h = model(
                batch_x_p=feature_h_init.clone(), batch_adj_p=adj_h.clone()
            )
    ...
    def train_one_epoch(
        ...
        for index in perm:
            ...
            for i in range(adj_p.size()[0]):
                drop_edge_p1[i] = model.aug_random_edge(
                    adj_p[i].cpu().numpy(), drop_edge1
                )
                drop_edge_p2[i] = model.aug_random_edge(
                    adj_p[i].cpu().numpy(), drop_edge2
                )
            ...
            for i in range(adj_h.size()[0]):
                ...
                drop_edge_h1[i] = model.aug_random_edge(
                    adj_h[i].cpu().numpy(), drop_edge1
                )
                drop_edge_h2[i] = model.aug_random_edge(
                    adj_h[i].cpu().numpy(), drop_edge2
                )
            ...
            if num % int(len(perm) / 10) == 0:
                print(
                    "\t Training Contrastive: {}/{}: index = {} loss = {}".format(
                        num, len(epoch_data), index, loss_sum
                    )
                )
    ...
    def eval_auc_epoch(model, eval_epoch_data):
        ...
        with torch.no_grad():
            ...
            for cur_data in eval_epoch_data:
                x1, x2, adj1, adj2, y = cur_data
                batch_output = model(
                    batch_x_p=x1, batch_x_h=x2, batch_adj_p=adj1, batch_adj_h=adj2
                )  # simscore
...
if __name__ == "__main__":
    ...
    title = "{}_Min{}_Max{}_InitDims{}_Task_{}_Filter_{}_Match_{}_P_{}_Agg_{}_Hidden_{}_Epoch_{}_Batch_{}_lr_{}_Dropout_{}_Global_{}_with_agg_{}".format(
        graph_name,
        graph_min,
        graph_max,
        graph_init_dim,
    ...
    main_log_dir = cfg_args.log_path + "{}_Min{}_Max{}_InitDims{}_Task_{}/".format(
        graph_name, graph_min, graph_max, graph_init_dim, cfg_args.task
    )
    ...
    sub_data_dir = "{}_{}ACFG_min{}_max{}".format(
        graph_name, graph_init_dim, graph_min, graph_max
    )
    cfg_data_dir = (
        os.path.join(main_data_dir, sub_data_dir)
        if "ffmpeg" in sub_data_dir
        else os.path.join(main_data_dir, sub_data_dir, "acfgSSL_6")
    )
    assert os.path.exists(cfg_data_dir), "the path of {} is not exist!".format(
        cfg_data_dir
    )

    cfg_trainer = CFGTrainer(
        node_init_dims=graph_init_dim,
        data_dir=cfg_data_dir,
        device=d,
        log_file=LOG_FILE,
        best_model_file=BestModel_FILE,
        best_cross_file=BestCross_FILE,
        args=cfg_args,
    )

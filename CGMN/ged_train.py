...
class GEDTrainer(object):
    ...
    def __init__(self, data_dir, device, best_model_path, args, log_path):
        ...
        self.model = HierarchicalGraphMatchNetwork(
            node_init_dims=self.dataset.input_dim, arguments=args, device=self.device
        ).to(self.device)
    ...
    def training_batch_predication(self, x1, adj1, x2, adj2, y):
        ...
        for i in range(adj_p.size()[0]):
            drop_edge_p1[i] = self.model.aug_random_edge(
                adj_p[i].cpu().numpy(), self.drop_edge1
            )
            drop_edge_p2[i] = self.model.aug_random_edge(
                adj_p[i].cpu().numpy(), self.drop_edge2
            )
        ...
        for i in range(adj_h.size()[0]):
            ...
            drop_edge_h1[i] = self.model.aug_random_edge(
                adj_h[i].cpu().numpy(), self.drop_edge1
            )
            drop_edge_h2[i] = self.model.aug_random_edge(
                adj_h[i].cpu().numpy(), self.drop_edge2
            )
            ...
            feature_p1, feature_htemp = self.model.matching_layer(
                feature_p1, feature_h0
            )

            feature_p2, feature_htemp = self.model.matching_layer(
                feature_p2, feature_h0
            )

            # ----------------------

            feature_h1, feature_ptemp = self.model.matching_layer(
                feature_h1, feature_p0
            )

            feature_h2, feature_ptemp = self.model.matching_layer(
                feature_h2, feature_p0
            )
    ...
    def val_batch_predication(
        ...
        assert (
            batch_feature_1.shape[0] == batch_adjacent_1.shape[0]
            and batch_feature_2.shape[0] == batch_adjacent_2.shape[0]
        )
        st = 0
    ...
    def testing_prediction(self):
        results = np.zeros(
            (len(self.dataset.testing_graphs), len(self.dataset.train_val_graphs))
        )
        ...
        for row in range(len(self.dataset.testing_graphs)):
            (
                ...
                batch_cols_adjacent,
                batch_cols_mask,
            ) = self.dataset.extract_test_matrices(row)
            st = 0
            pred = []
            predictions = []
            while st < len(self.dataset.train_val_graphs):
                ...
                with torch.no_grad():
                    ...
                    feature_p_init = torch.FloatTensor(batch_rows_feature_small).to(
                        self.device
                    )
                    adj_p = torch.FloatTensor(batch_rows_adjacent_small).to(self.device)

                    feature_h_init = torch.FloatTensor(batch_cols_feature_small).to(
                        self.device
                    )
    ...
    def fit(self):
        ...
        for iteration in range(self.max_iterations):
            ...
            train_loss = self.training_batch_predication(
                batch_feature_1, batch_adj_1, batch_feature_2, batch_adj_2, batch_ged
            )
            ...
            if iteration % int(self.max_iterations / 10) == 0:
                ...
                write_log_file(
                    self.log_file,
                    "Iteration = {}\n\tbatch loss={}, @ {}".format(
                        iteration, train_loss, time_spent
                    ),
                )
            ...
            if self._need_val(iteration=iteration):
                ...
                val_loss, time_spent = self.val_batch_predication(
                    val_feature_1,
                    val_adj_1,
                    val_mask_1,
                    val_feature_2,
                ...
                write_log_file(
                    self.log_file,
                    "\nvalidation iteration={}, loss={}, spend time = {} @ {}".format(
                        iteration, val_loss * 1000, time_spent, datetime.now()
                    ),
                )
                ...
                write_log_file(
                    self.log_file,
                    "\tbest validation mse = {} (e-3)".format(best_val_loss * 1000),
                )
        return epoch
    ...
    def testing(self):
        ...
        write_log_file(
            self.log_file,
            "\nDouble check validation, loss = {}(e-3) @ {}".format(
                val_loss * 1000, datetime.now()
            ),
        )

        test_predictions = self.testing_prediction()
        test_mse = metrics_mean_square_error(
            self.dataset.ground_truth.flatten(), test_predictions.flatten()
        )
        test_rho = metrics_spearmanr_rho(
            self.dataset.ground_truth.flatten(), test_predictions.flatten()
        )
        test_tau = metrics_kendall_tau(
            self.dataset.ground_truth.flatten(), test_predictions.flatten()
        )
        ps, inclusive_true_ks, inclusive_pred_ks = computing_precision_ks(
            trues=self.dataset.ground_truth,
            predictions=test_predictions,
            ks=[10, 20],
            inclusive=self.flag_inclusive,
            rm=0,
        )
        test_results = {
            "mse": test_mse,

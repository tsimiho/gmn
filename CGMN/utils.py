...
def generate_epoch_pair(graphs, classes, batch, output_id=False, load_id=None):
    ...
    if load_id is None:
        ...
        while st < len(graphs):
            if output_id:
                input1, input2, adj1, adj2, y, pos_id, neg_id = get_pair(
                    graphs, classes, batch, st=st, output_id=True
                )
        ...
        for id_pair in id_data:
            input1, input2, adj1, adj2, y = get_pair(
                graphs, classes, batch, load_id=id_pair
            )
            epoch_data.append((input1, input2, adj1, adj2, y))

...
class AIDSData(Data):
    def __init__(self, save_folder, train_or_test):
        ...
        data_dir = "{}/{}/{}".format(
            self.get_save_folder(), self.get_dataset_folder_name(), train_or_test
        )
        self.graphs = []
        ...
        print(
            "The max number of nodes among all graphs of {} is # {}".format(
                self.get_dataset_folder_name(), self.get_max_number_of_nodes()
            )
        )
...
class IMDBData(Data):
    def __init__(self, save_folder, train_or_test):
        super(IMDBData, self).__init__(save_folder=save_folder)
        # load all graphs
        data_dir = "{}/{}/{}".format(
            self.get_save_folder(), self.get_dataset_folder_name(), train_or_test
        )
...
class SiameseDataSet(object):
    def __init__(
        ...
        orig_train_data = self.load_data_set(
            data=self.data_set_name, train_or_test="train"
        )
        ...
        print(
            "\t\ttrain_gs.len={} and val_gs.len={}, and their max number of nodes = {}".format(
                len(self.train_gs), len(self.val_gs), self.train_val_max_number_of_nodes
            )
        )
        print("\nprepare the testing data set")
        orig_test_data = self.load_data_set(
            data=self.data_set_name, train_or_test="test"
        )
        self.test_gs = orig_test_data.graphs
        self.test_max_number_of_nodes = orig_test_data.get_max_number_of_nodes()

        print(
            "\t\ttest_gs.len={}, and their max number of nodes = {}".format(
                len(self.test_gs), self.test_max_number_of_nodes
            )
        )

        self.node_feat_encoder = self._get_node_feature_encoder(
            orig_train_data.graphs + self.test_gs
        )

    def load_data_set(self, data, train_or_test):
        if data.lower() == "aids700nef":
            return AIDSData(
                save_folder=self.data_save_folder, train_or_test=train_or_test
            )
        elif data.lower() == "linux":
            return LinuxData(
                save_folder=self.data_save_folder, train_or_test=train_or_test
            )
        elif data.lower() == "imdbmulti":
            return IMDBData(
                save_folder=self.data_save_folder, train_or_test=train_or_test
            )
        else:
    ...
    def _train_val_split(self, orig_train_data):
        if self.validation_ratio < 0 or self.validation_ratio > 1:
            raise RuntimeError(
                "ratio of validation {} must be in [0, 1]".format(self.validation_ratio)
            )
    ...
    def _get_node_feature_encoder(self, graphs):
        if self.node_encoder_name == "OneHot":
            ...
            return NodeFeatureConstantEncoder(
                graphs, self.node_feat_name, self.node_encoder_name
            )
        else:
            raise RuntimeError(
                "Unknown node_feat_encoder {}".format(self.node_feat_encoder)
            )
...
class NodeFeatureConstantEncoder(NodeFeatureEncoder):
    ...
    def __init__(self, _, node_feat_name=None, node_encoder_name="constant_1_2"):
        ...
        print(
            "A constant feature encoder where the input_dim is {} and input feature is {}".format(
                self.input_dim_, self.const
            )
        )
...
def computing_precision_ks(trues, predictions, ks, inclusive=True, rm=0):
    ...
    return (
        np.mean(precision_ks, axis=0),
        np.mean(inclusive_final_true_ks, axis=0),
        np.mean(inclusive_final_pred_ks, axis=0),
    )
...
class DistanceCalculator(object):
    ...
    def __init__(self, root_folder, data_name):
        ...
        self.sfn = "{}/{}_{}_{}_gidpair_dist_map".format(
            root_folder, data_name.lower(), "ged", "astar"
        )
        ...
        if not self.gid_pair_dist_map:
            ...
            print(
                "Loaded dist map from {} with {} entries".format(
                    self.sfn, len(self.gid_pair_dist_map)
                )
            )
...
class DistanceModelResult(Result):
    def __init__(
        ...
        for i in range(m):
            for j in range(n):
                self.dist_norm_mat_[i][j] = normalized_exp_kernel_ged(
                    self.dist_mat_[i][j], row_graphs[i], col_graphs[j], scale
                )
    ...
    def mat(self, metric, norm):
        if metric == self.dist_metric():
            ...
            raise RuntimeError(
                "Unknown metric {} for model {}".format(metric, self.model_)
            )
    ...
    def _load_result_mat(self, metric, model, m, n):
        file_p = self.result_folder + "/{}/{}/{}_{}_mat_{}_{}_*.npy".format(
            self.dataset, metric, self.dist_metric(), metric, self.dataset, model
        )
    ...
    def _load_merged_astar_from_other_three(self, metric, m, n):
        ...
        dms = [
            self._load_result_mat(self.dist_metric(), model, m, n)
            for model in merge_models
        ]
        tms = [self._load_result_mat("time", model, m, n) for model in merge_models]
    ...
    def _choose_result_file(self, files, m, n):
        ...
        if cands:
            return cands[0]
        raise RuntimeError(
            "No result files in {}".format(files)
        )  # TODO: smart choice and cross-checking
...
def load_result(
    ...
    if "beam" in model or model in ["astar", "hungarian", "vj"]:
        return PairwiseGEDModelResult(
            dataset, model, result_folder, dist_mat, row_graphs, col_graphs, scale
        )

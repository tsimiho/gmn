...
def compute_cross_attention_scores(x, y, sim):
    """Compute cross-graph attention scores.

    Args:
      x: [N, D] float tensor, node representations from one graph.
      y: [M, D] float tensor, node representations from the other graph.
      sim: a function that computes similarity between x and y.

    Returns:
      attention_scores: [N, M] float tensor, attention scores from x to y.
    """
    # Compute similarity scores
    scores = sim(x, y)

    # Normalize scores to get attention scores

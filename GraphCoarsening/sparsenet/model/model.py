...
from sparsenet.util.util import fix_seed, random_pygeo_graph, summary
...
class GINConv(MessagePassing):
    ...
    def __init__(self, edge_feat_dim, emb_dim, aggr="add"):
        ...
        self.edge_embedding = torch.nn.Linear(
            self.edge_feat_dim, emb_dim
        )  # torch.nn.Embedding(num_bond_type, emb_dim)
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        return self.propagate(
            edge_index, size=[x.size(0), x.size(0)], x=x, edge_attr=edge_embeddings
        )
...
class GNN(torch.nn.Module):
    ...
    def forward(self, *argv):
        ...
        x = self.x_embedding0(
            x
        )  # self.x_embedding1(x[:, 0]) + self.x_embedding2(x[:, 1])
    ...
    def forward(self, *argv, ini=False):
        ...
        if self.force_pos:
            if ini:
                ...
                return 1 + torch.nn.ReLU()(
                    rep
                )  # important: add 1 by default. not sure it's the best.
        else:
            return rep
...
if __name__ == "__main__":
    ...
    model = GNN_graphpred(
        n_layer, emb_dim, node_feat_dim, edge_feat_dim, out_dim, mlp=False
    )

    g1 = random_pygeo_graph(n_node, node_feat_dim, n_edge, edge_feat_dim, device="cpu")
    g2 = random_pygeo_graph(
        n_node + 10, node_feat_dim, n_edge + 10, edge_feat_dim, device="cpu"
    )
    summary(g1, "g1")
    loader = DataLoader(
        [g1] * 16 + [g2] * 16, batch_size=8, shuffle=True, num_workers=0
    )
    for batch in loader:
        pred = model(batch)
        summary(pred, "pred")

...
class PlotUtils:
    ...
    def plot_custom_dataset(self, graph, nodelist, x, figname):
        print(f"Plotting custom dataset graph with {len(graph.nodes)} nodes.")
        ...
        # Highlight the selected subgraph nodes (nodelist)
        nx.draw_networkx_nodes(
            graph, pos, nodelist=nodelist, node_color="red", node_size=700
        )

        plt.title("Custom Dataset Graph")
        plt.savefig(figname)
        plt.close()
    ...
    def plot_soft_edge_mask(
        self, graph, edge_index, edge_mask, top_k, un_directed, figname, **kwargs
    ):
        ...
        if self.dataset_name.lower() == "BA_2motifs".lower():
            ...
            self.plot_sentence(
                graph, nodelist, words=words, edgelist=edgelist, figname=figname
            )
        else:
            raise NotImplementedError
